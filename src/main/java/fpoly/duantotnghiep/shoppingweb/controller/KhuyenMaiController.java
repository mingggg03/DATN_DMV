...
public class KhuyenMaiController {
    ...
    private final IKhuyenMaiService iKhuyenMaiService;
    ...
    @GetMapping("")
    public String hienThi(Model model, @RequestParam(name = "pageNumber", required = false, defaultValue = "1") int pageNumber) {
        Page<KhuyenMaiResponse> page = iKhuyenMaiService.findAll(pageNumber - 1, 8);
        List<KhuyenMaiResponse> list = page.getContent();
        model.addAttribute("khuyenMai", list);
        model.addAttribute("khuyenMaiRequestFilter", new KhuyenMaiRequestFilter());
    ...
    @PostMapping("/add")
    public String add(@Valid @ModelAttribute("khuyenMai") KhuyenMaiRequest khuyenMaiRequest, BindingResult result, Model model, @RequestParam(value = "ids", required = false) List<String> sanPham) {
        validateNhap(result, khuyenMaiRequest);
        if (sanPham != null && khuyenMaiRequest.getMucGiam() != null) {
            List<SanPhamModel> sanPhamModel = iSanPhamRepository.findByMaIn(sanPham);
            sanPhamModel.forEach(x -> {
                if (khuyenMaiRequest.getMucGiam().compareTo(x.getGiaBan()) > 0) {
                    result.rejectValue("mucGiam", "mucGiamError", "Mức giảm không thể áp dụng cho sản phẩm " + x.getMa() + " - " + x.getTen());
                }
            });
        ...
        iKhuyenMaiService.save(khuyenMaiRequest);
        return "redirect:/admin/khuyen-mai";
    }

    @GetMapping("/{id}")
    public String findById(@PathVariable("id") String id, Model model) {
        List<String> sanPhamOn = iKhuyenMaiService.findById(id).getSanPham().stream().map(SanPhamModel::getMa).collect(Collectors.toList());
        List<String> sanPhamKhuyenMai = iSanPhamService.findByAllSanPhamWithKM().stream().map(SanPhamModel::getMa).collect(Collectors.toList());
        model.addAttribute("khuyenMai", iKhuyenMaiService.findById(id));
        model.addAttribute("id", iKhuyenMaiService.findById(id).getMa());
        model.addAttribute("ct", "aaa");
        model.addAttribute("tt", iKhuyenMaiService.findById(id).getTrangThai());
        int dis = 1;
        if (iKhuyenMaiService.findById(id).getNgayBatDau().after(new Date()) || iKhuyenMaiService.findById(id).getNgayKetThuc().before(new Date())) {
            dis = 0;
        }
        model.addAttribute("dis", dis);
        model.addAttribute("sanPham", iSanPhamRepository.findAll());
        model.addAttribute("action", "/admin/khuyen-mai/update/" + id);
        model.addAttribute("sanPhamOn", sanPhamOn);
        model.addAttribute("sanPhamKhuyenMai", listSanPhamKhuyenMaiById(sanPhamKhuyenMai, sanPhamOn));
        return "/admin/formKhuyenMai";
    }

    @PostMapping("/update/{id}")
    public String update(@Valid @ModelAttribute("khuyenMai") KhuyenMaiRequest khuyenMaiRequest, BindingResult result, Model model, @PathVariable String id, @RequestParam(value = "ids", required = false) List<String> sanPham) {
        validateNhap(result, khuyenMaiRequest);
        if (sanPham != null && khuyenMaiRequest.getMucGiam() != null) {
            List<SanPhamModel> sanPhamModel = iSanPhamRepository.findByMaIn(sanPham);
            sanPhamModel.forEach(x -> {
                if (khuyenMaiRequest.getMucGiam().compareTo(x.getGiaBan()) > 0) {
                    result.rejectValue("mucGiam", "mucGiamError", "Mức giảm không thể áp dụng cho một số sản phẩm.");
                }
            });
            khuyenMaiRequest.setSanPham(sanPhamModel);
        }
        ...
        KhuyenMaiResponse km = iKhuyenMaiService.findById(id);
    ...
    public String capKichHoat(@PathVariable("id") String id) {
        KhuyenMaiModel khuyenMaiModel = iKhuyenMaiService.findById1(id);
        khuyenMaiModel.setTrangThai(0);
        iKhuyenMaiService.capNhatTrangThai(khuyenMaiModel);
        return "redirect:/admin/khuyen-mai/" + id;
    }

    @GetMapping("/dung-kich-hoat/{id}")
    public String capDungKichHoat(@PathVariable("id") String id) {
        KhuyenMaiModel khuyenMaiModel = iKhuyenMaiService.findById1(id);
    ...
    private void getFormAdd(Model model, KhuyenMaiRequest khuyenMaiRequest) {
        ...
        model.addAttribute("sanPham", iSanPhamRepository.findAll());
        model.addAttribute("sanPhamKhuyenMai", iSanPhamService.findByAllSanPhamWithKM().stream().map(SanPhamModel::getMa).collect(Collectors.toList()));
    ...
    private static void validateNhap(BindingResult result, KhuyenMaiRequest khuyenMaiRequest) {
        if (khuyenMaiRequest.getLoai().equals("TIEN")) {
            if (khuyenMaiRequest.getMucGiam() != null) {
                if (khuyenMaiRequest.getMucGiam().compareTo(new BigDecimal("1000")) < 0) {
                    result.rejectValue("mucGiam", "mucGiamError", "Mức giảm phải lớn hơn 1000.");
                }
            }
            ...
            if (khuyenMaiRequest.getMucGiam() != null) {
                if (khuyenMaiRequest.getMucGiam().compareTo(new BigDecimal("1")) < 0 || khuyenMaiRequest.getMucGiam().compareTo(new BigDecimal("99")) > 0) {
                    result.rejectValue("mucGiam", "mucGiamError", "Mức giảm phải trong khoảng 1-99.");
                }
            }
        }
        if (khuyenMaiRequest.getNgayBatDau() != null && khuyenMaiRequest.getNgayKetThuc() != null) {
            if (khuyenMaiRequest.getNgayBatDau().after(khuyenMaiRequest.getNgayKetThuc())) {
                result.rejectValue("ngayBatDau", "ngayBatDauError", "Ngày bắt đầu phải trước ngày kết thúc.");
            }
        }
    }
    ...
    public String loc(@ModelAttribute("khuyenMaiRequestFilter") KhuyenMaiRequestFilter khuyenMaiRequestFilter, @RequestParam(defaultValue = "1", name = "pageNumber", required = false) Integer pageNumber, Model model) {
        Page<KhuyenMaiResponse> page = iKhuyenMaiService.locKM(khuyenMaiRequestFilter, pageNumber - 1, 5);
        List<KhuyenMaiResponse> list = page.getContent();
        model.addAttribute("khuyenMai", list);
        model.addAttribute("totalPage", page.getTotalPages());
        model.addAttribute("currentPage", pageNumber);
    ...
    private void updateGiaKhiGiam(KhuyenMaiResponse khuyenMai, List<SanPhamModel> sanPhamUpdate) {
        for (SanPhamModel sanPhamModel1 : sanPhamUpdate) {
            ...
            iSanPhamService.save1(sanPhamModel1);
        }
    }
}
